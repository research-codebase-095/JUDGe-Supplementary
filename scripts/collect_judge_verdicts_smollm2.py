"""Third judge-model-family counterpart to scripts/collect_judge_verdicts.py
and collect_judge_verdicts_1p5b.py: same exact methodology (same dataset/
filter, same prompt template, same single-forward-pass raw-logit verdict
extraction, same featurize()/three_way_split/DeLong protocol), but a
genuinely different model family (HuggingFaceTB/SmolLM2-1.7B-Instruct, not
another Qwen size) - added to answer whether the existing judge pilot's
near-tied features / non-significant combiner / standardization-does-not-
help pattern is a property of judge reliability in general or an accident of
one model family (Qwen), per GAP 1's "model-specific or benchmark-specific"
question.

Model choice: ungated (no HF auth/license acceptance needed, unlike
Llama-3.2 or Gemma-2), standard AutoModelForCausalLM/AutoTokenizer + chat
template support (works through load_frozen_causal_lm unchanged), and at
1.7B params/float32 (~7GB) directly comparable in memory cost to the 1.5B
Qwen run that already completed successfully on this machine's 16.5GB RAM
ceiling (see llm_backbone.py's load_frozen_causal_lm docstring for why
float32 is forced and must not change).

Unlike the two Qwen scripts, TOKEN_ID_A/TOKEN_ID_B are NOT hardcoded here -
those were Qwen-tokenizer-specific magic numbers (32/33), and nothing
guarantees a different tokenizer assigns "A"/"B" the same ids. They are
resolved at runtime via a direct encode() call, asserted to be single tokens,
and re-checked against the top-5 sanity printout before being trusted for
the full loop - the same discipline collect_judge_verdicts_1p5b.py used to
*re-verify* an assumption carried over from the 0.5B run, generalized here
to a case where there is no prior assumption to carry over at all.

Checkpointing: identical pattern to the two Qwen scripts, distinct
checkpoint filename so it cannot collide with either.

Usage:
    python scripts/collect_judge_verdicts_smollm2.py
"""
...
def resolve_token_ids(tokenizer) -> tuple[int, int]:
    ...
    encode() calls, rather than assuming Qwen's 32/33 - a different model
    family has no reason to share those ids. Fails loudly if "A"/"B" is not
    a single token under this tokenizer, rather than silently mis-indexing."""
...
def sanity_check_tokens(model, tokenizer, rows, token_id_a: int, token_id_b: int, n_check: int = 5) -> None:
    ...
    checked against actual model behavior (do they even show up near the top
    of the distribution?) before being trusted for the full run."""
...
CHECKPOINT_PATH = os.path.join(DATA_DIR, "_judge_checkpoint_mtbench_smollm2.pt")
CHECKPOINT_EVERY = 100  # examples between checkpoint saves - see collect_judge_verdicts.py's comment for why this exists


def main(limit: int | None) -> None:
    out_cache_path = os.path.join(DATA_DIR, "judge_feature_cache_mtbench_smollm2.pt")
    out_results_path = os.path.join(DATA_DIR, "judge_experiment_results_smollm2.json")
