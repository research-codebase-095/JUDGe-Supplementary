"""1.5B-parameter counterpart to scripts/collect_judge_verdicts.py: same exact
methodology (same dataset/filter, same prompt template, same single-forward-
pass raw-logit verdict extraction, same featurize()/three_way_split/DeLong
protocol), only the judge model changes, from Qwen2.5-0.5B-Instruct to
Qwen2.5-1.5B-Instruct (3x the parameters), to see whether a meaningfully
larger judge in the same model family does any better than a near-chance
0.515 accuracy / 0.552 MSP AUROC / 0.574 combiner AUROC (not significant).

Model-size choice: this machine has only 16.5GB total RAM. Qwen2.5-1.5B-
Instruct at float32 (load_frozen_causal_lm forces float32 - see that
function's docstring for the real Pythia-160m float16-overflow bug it fixes;
do not change it) needs ~6GB for weights alone, which fits with real headroom
unlike 3B (~12GB) or 7B (~28GB, categorically impossible here) - see this
script's own memory-check comments in main() for what was actually observed
on this run.

Checkpointing: identical pattern to collect_judge_verdicts.py (see that
script's comment in main() for why it exists), but under a distinct
checkpoint filename so it cannot collide with the 0.5B run's checkpoint or
with the unrelated swap-consistency job's checkpoint.

Usage:
    python scripts/collect_judge_verdicts_1p5b.py
"""
...
# Qwen2.5 models share a tokenizer/vocab across sizes, so TOKEN_ID_A=32 /
# TOKEN_ID_B=33 (confirmed for the 0.5B model in collect_judge_verdicts.py)
# are expected to hold here too - but this is re-verified below, both via a
# direct encode() check and via sanity_check_tokens' top-5 top-token check on
# 5 real examples, before being trusted for the full loop. See main()'s
# printed output / this run's final report for the actual outcome.
TOKEN_ID_A = 32  # tokenizer.encode("A", add_special_tokens=False) - re-verified below
TOKEN_ID_B = 33  # tokenizer.encode("B", add_special_tokens=False) - re-verified below
...
def verify_token_ids(tokenizer) -> None:
    """Direct encode() re-verification of TOKEN_ID_A/TOKEN_ID_B for this
    model's tokenizer, before anything else runs. Raises if the assumption
    carried over from the 0.5B run does not hold for this model, rather than
    silently producing a broken run."""
    ...
    assert enc_a == [TOKEN_ID_A], f"TOKEN_ID_A={TOKEN_ID_A} does not match encode('A')={enc_a} for {MODEL_NAME}"
    assert enc_b == [TOKEN_ID_B], f"TOKEN_ID_B={TOKEN_ID_B} does not match encode('B')={enc_b} for {MODEL_NAME}"
    print("TOKEN_ID_A/TOKEN_ID_B confirmed via direct encode() for this tokenizer.")
    print()
...
CHECKPOINT_PATH = os.path.join(DATA_DIR, "_judge_checkpoint_mtbench_1p5b.pt")
CHECKPOINT_EVERY = 100  # examples between checkpoint saves - see collect_judge_verdicts.py's comment for why this exists


def main(limit: int | None) -> None:
    out_cache_path = os.path.join(DATA_DIR, "judge_feature_cache_mtbench_1p5b.pt")
    out_results_path = os.path.join(DATA_DIR, "judge_experiment_results_1p5b.json")
