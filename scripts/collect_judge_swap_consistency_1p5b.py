"""1.5B-parameter counterpart to scripts/collect_judge_swap_consistency.py:
identical methodology (same dataset/filter, same prompt template, same
verdict re-mapping under order-swap, same 6-feature-combiner-vs-5-feature
DeLong protocol), only the judge model changes, from Qwen2.5-0.5B-Instruct
to Qwen2.5-1.5B-Instruct.

Motivation: judge2026.tex's Discussion section argues 0.5B is a near-total
position-following degenerate (99.5% model_a regardless of content) and that
1.5B, which beats the position baseline by 4.4 points, is the more
informative configuration for testing whether the reliability-vector null
generalizes - yet the paper's swap-consistency (explicit order-swap) test had
only ever been run at 0.5B. This closes that gap at 1.5B specifically.

Reuses collect_judge_verdicts_1p5b.py's build_prompt(), load_filtered_dataset(),
TOKEN_ID_A/TOKEN_ID_B, verdict_logits(), and MODEL_NAME by importing that
module directly - not re-deriving any of this, exactly mirroring how
collect_judge_swap_consistency.py reuses collect_judge_verdicts.py's.

Reuses judge_feature_cache_mtbench_1p5b.pt's `splits` field directly so the
6-feature combiner is fit/evaluated on the EXACT same combiner_fit/id_test
row partition as the existing 1.5B 5-feature combiner (required for the
DeLong paired comparison to be valid).

Checkpointing mirrors the 0.5B swap-consistency script's pattern exactly.

Usage:
    python scripts/collect_judge_swap_consistency_1p5b.py
"""
...
import collect_judge_verdicts_1p5b as cjv15  # noqa: E402 - reuse build_prompt/load_filtered_dataset/TOKEN_ID_A/B/verdict_logits/MODEL_NAME

DATA_DIR = os.path.join(REPO_ROOT, "data")
CHECKPOINT_PATH = os.path.join(DATA_DIR, "_judge_checkpoint_swap_consistency_1p5b.pt")
CHECKPOINT_EVERY = 100  # examples between checkpoint saves - matches the 0.5B swap-consistency script


def main(limit: int | None) -> None:
    out_cache_path = os.path.join(DATA_DIR, "judge_swap_consistency_cache_1p5b.pt")
    out_results_path = os.path.join(DATA_DIR, "judge_swap_consistency_results_1p5b.json")
    existing_cache_path = os.path.join(DATA_DIR, "judge_feature_cache_mtbench_1p5b.pt")
    existing_results_path = os.path.join(DATA_DIR, "judge_experiment_results_1p5b.json")

    filtered = cjv15.load_filtered_dataset()
    rows = list(filtered)
    ...
    if limit is None:
        ...
        print("row alignment against judge_feature_cache_mtbench_1p5b.pt verified: "
    ...
    print(f"loading frozen {cjv15.MODEL_NAME}...")
    model, tokenizer = load_frozen_causal_lm(cjv15.MODEL_NAME)
    ...
    for i in range(start_idx, len(rows)):
        ...
        prompt_orig = cjv15.build_prompt(tokenizer, instruction, response_a, response_b)
        logits_orig = cjv15.verdict_logits(model, tokenizer, prompt_orig)
        pred_orig = "model_a" if logits_orig[cjv15.TOKEN_ID_A].item() > logits_orig[cjv15.TOKEN_ID_B].item() else "model_b"

        prompt_swap = cjv15.build_prompt(tokenizer, instruction, response_b, response_a)
        logits_swap = cjv15.verdict_logits(model, tokenizer, prompt_swap)
        pred_swap = "model_b" if logits_swap[cjv15.TOKEN_ID_A].item() > logits_swap[cjv15.TOKEN_ID_B].item() else "model_a"
    ...
    if limit is None:
        ...
        print(f"determinism check vs judge_feature_cache_mtbench_1p5b.pt's `correct`: "
    ...
    # Cross-tab against the paper's existing 1.5B failure-mode rules
    # (high-confidence-wrong / low-confidence-correct), matching the 0.5B
    # cross-tab already reported in the paper - only meaningful if
    # swap-consistency's rate at 1.5B leaves enough positive cells.
    from scipy.stats import fisher_exact  # noqa: E402

    msp_test = phi5_test[:, 0].numpy()
    correct_test_np = correct5_test.numpy()
    q75, q25 = float(msp_test.__class__.__mro__ and __import__("numpy").percentile(msp_test, 75)), float(__import__("numpy").percentile(msp_test, 25))
    swap_test_np = swap_consistent_t[id_test_idx].numpy().astype(bool)
    hcw_mask = (~correct_test_np) & (msp_test >= q75)
    lcc_mask = (correct_test_np) & (msp_test <= q25)
    n_hcw, n_lcc = int(hcw_mask.sum()), int(lcc_mask.sum())
    n_swap_consistent_test = int(swap_test_np.sum())

    print()
    print("=== Cross-tab: swap-consistency vs failure modes (1.5B, id_test) ===")
    print(f"swap-consistent in id_test: {n_swap_consistent_test}/{len(swap_test_np)}")
    if n_swap_consistent_test >= 5:
        hcw_swap = int((hcw_mask & swap_test_np).sum())
        table_hcw = [[hcw_swap, n_hcw - hcw_swap], [n_swap_consistent_test - hcw_swap, len(swap_test_np) - n_hcw - (n_swap_consistent_test - hcw_swap)]]
        _, p_hcw = fisher_exact(table_hcw)
        print(f"high-confidence-wrong: n={n_hcw}, swap-consistent within rule={hcw_swap}/{n_hcw}, Fisher p={p_hcw:.4g}")

        lcc_swap = int((lcc_mask & swap_test_np).sum())
        table_lcc = [[lcc_swap, n_lcc - lcc_swap], [n_swap_consistent_test - lcc_swap, len(swap_test_np) - n_lcc - (n_swap_consistent_test - lcc_swap)]]
        _, p_lcc = fisher_exact(table_lcc)
        print(f"low-confidence-correct: n={n_lcc}, swap-consistent within rule={lcc_swap}/{n_lcc}, Fisher p={p_lcc:.4g}")
    else:
        print(f"too few swap-consistent examples ({n_swap_consistent_test}) for a powered cross-tab - reporting counts only, no test.")
    ...
    results = {
        ...
        "orig_a_frac": orig_a_frac,
        "swap_a_frac": swap_a_frac,
    }
